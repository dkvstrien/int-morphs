...
from .intmorphs_config import IntMorphsConfig
from .morpheme import Morpheme
from .name_file_utils import get_names_from_file_as_morphs
from .recalc.anki_data_utils import IntMorphsCardData


class IntMorphsDB:  # pylint:disable=too-many-public-methods
    ...
    def __init__(self, db_path: Path | None = None) -> None:
        ...
        if db_path is None:
            db_path = Path(mw.pm.profileFolder(), "intmorphs.db")

        self.con: sqlite3.Connection = sqlite3.connect(db_path)

    def __enter__(self) -> IntMorphsDB:
        """
    ...
    def create_all_tables(self) -> None:
        ...
        self.create_lemma_confidence_table()
        ...
        with self.con:
            ...
            for row in card_morphs_raw:
                ...
                if interval >= am_config.interval_for_known_morphs:
                    learning_status = intmorphs_globals.STATUS_KNOWN
                elif interval > 0:
                    learning_status = intmorphs_globals.STATUS_LEARNING
                else:
                    learning_status = intmorphs_globals.STATUS_UNKNOWN
    ...
    def get_am_cards_data_dict(
        ...
        am_db_row_data_dict: dict[CardId, IntMorphsCardData] = {}
        for am_data in map(IntMorphsCardData, result):
    ...
    def rebuild_seen_morphs_today() -> None:
        ...
        operation = QueryOp(
            parent=mw,
            op=lambda _: IntMorphsDB.rebuild_seen_morphs_today_background(),
    ...
    def rebuild_seen_morphs_today_background() -> None:
        ...
        am_db = IntMorphsDB()
        cards_studied_today: Sequence[int] = IntMorphsDB.get_new_cards_seen_today()
    ...
    # ─── Bayesian Confidence CRUD ───────────────────────────────────────

    def get_lemma_confidence_dict(self) -> dict[str, "LemmaConfidence"]:
        """Load all lemma confidences from DB into a dict."""
        from .bayesian.models import LemmaConfidence, ConfidenceStage

        result: dict[str, LemmaConfidence] = {}
        try:
            rows = self.con.execute(
                "SELECT * FROM lemma_confidence"
            ).fetchall()
            for row in rows:
                lc = LemmaConfidence(
                    lemma=row["lemma"],
                    score=row["score"],
                    stage=ConfidenceStage(row["stage"]),
    ...
    def upsert_lemma_confidence(self, lc: "LemmaConfidence") -> None:
        """Insert or update a LemmaConfidence in the DB."""
        import time
        ...
        with self.con:
            self.con.execute(
                ...
                """,
                (
                    lc.lemma, lc.score, lc.stage.value,
                    lc.appearances, lc.pass_count, lc.fail_count,
    ...
    def save_all_lemma_confidences(
        self, confidence_map: dict[str, "LemmaConfidence"]
    ) -> int:
        """Save all lemma confidences to DB. Returns count saved."""
        saved = 0
        for lemma, lc in confidence_map.items():
    ...
    def get_new_cards_seen_today() -> Sequence[int]:
        ...
        #   "(introduced:1 note:intmorphs\_sub2srs OR introduced:1 note:Basic) OR is:buried tag:am-known-manually"
        assert mw is not None
