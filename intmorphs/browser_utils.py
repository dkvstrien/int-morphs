...
from . import intmorphs_config, intmorphs_globals
...
def browse_same_morphs(  # pylint:disable=too-many-arguments
    ...
    am_db = IntMorphsDB()
    am_filter = intmorphs_config.get_matching_read_filter(note)
...
def browse_study_morphs_for_highlighted_morph(selected_text: str) -> None:
    ...
    # ids of cards in the IntMorphsDB, this function does a query
    # for the text
    global browser
    assert mw is not None
    ...
    query = f'"{intmorphs_globals.EXTRA_FIELD_STUDY_MORPHS}:{escaped_selected_text}"'
    search_edit.setText(query)
    browser.onSearchActivated()
...
def run_view_morphs() -> None:  # pylint:disable=too-many-locals
    ...
    for cid in browser.selectedCards():
        ...
        am_config_filter: IntMorphsConfigFilter | None = (
            intmorphs_config.get_matching_read_filter(note)
        )
