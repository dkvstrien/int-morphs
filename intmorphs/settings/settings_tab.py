...
class SettingsTab(ABC):  # pylint:disable=too-many-instance-attributes
    ...
    def _want_to_reset_am_tags(self, reason_for_reset: str) -> bool:
        title = "Reset Tags?"
        body = (
            f"Switching the {reason_for_reset} may cause some existing IntMorphs "
            "tags to become misleading, and removing them is therefore recommended.<br><br>"
            "Would you like IntMorphs to remove the following tags from your cards now?"
            "<ul>"
    ...
    def populate(self, use_default_config: bool = False) -> None:
        source_object: IntMorphsConfig = self._config
