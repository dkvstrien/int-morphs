...
#                          IMPORTS
################################################################
# We have to use implicit imports from the "intmorphs" module
...
# from intmorphs import browser_utils
################################################################
...
from . import intmorphs_globals as am_globals
from . import (
    browser_utils,
    debug_utils,
    message_box_utils,
...
from .bayesian.review_processor import process_bayesian_review
from .intmorphs_config import IntMorphsConfig, IntMorphsConfigFilter
from .intmorphs_db import IntMorphsDB
from .extra_settings import intmorphs_extra_settings, extra_settings_keys
from .extra_settings.intmorphs_extra_settings import IntMorphsExtraSettings
...
def main() -> None:
    ...
    gui_hooks.reviewer_did_answer_card.append(process_bayesian_review)
...
def load_am_profile_configs() -> None:
    ...
    try:
        with open(profile_settings_path, encoding="utf-8") as file:
            ...
            intmorphs_config.load_stored_am_configs(profile_settings)
    except FileNotFoundError:
        # This is reached when we load a new anki profile that hasn't saved
        # any intmorphs settings yet. It's important that we don't carry over
        ...
        intmorphs_config.reset_all_configs()
...
def init_browser_menus_and_actions() -> None:
    ...
    def setup_browser_menu(_browser: Browser) -> None:
        ...
        am_browse_menu = QMenu("IntMorphs", mw)
...
def maybe_show_version_warning() -> None:
    ...
    am_extra_settings = IntMorphsExtraSettings()
...
def update_seen_morphs(_overview: Overview) -> None:
    ...
    read_config_filters: list[IntMorphsConfigFilter] = (
        intmorphs_config.get_read_enabled_filters()
    )
    ...
    if has_active_note_filter:
        IntMorphsDB.rebuild_seen_morphs_today()
...
def cleanup_profile_session() -> None:
    ...
    IntMorphsDB.drop_seen_morphs_table()
    IntMorphsExtraSettings().save_current_intmorphs_version()
...
def create_am_tool_menu() -> QMenu:
    assert mw is not None
    am_tool_menu = QMenu("IntMorphs", mw)
...
def create_recalc_action(am_config: IntMorphsConfig) -> QAction:
...
def create_settings_action(am_config: IntMorphsConfig) -> QAction:
...
def create_learn_now_action(am_config: IntMorphsConfig) -> QAction:
...
def create_browse_same_morph_unknowns_action(am_config: IntMorphsConfig) -> QAction:
...
def create_view_morphs_action(am_config: IntMorphsConfig) -> QAction:
...
def create_already_known_tagger_action(am_config: IntMorphsConfig) -> QAction:
...
def add_text_as_name_action(web_view: AnkiWebView, menu: QMenu) -> None:
    ...
    action.triggered.connect(IntMorphsDB.insert_names_to_seen_morphs)
...
def create_generators_dialog_action(am_config: IntMorphsConfig) -> QAction:
...
def create_progression_dialog_action(am_config: IntMorphsConfig) -> QAction:
...
def create_known_morphs_exporter_action(am_config: IntMorphsConfig) -> QAction:
...
def test_function() -> None:
    ...
    # 1. In intmorphs_globals.py set 'DEV_MODE = True'
