...
# a file ("intmorphs_profile_settings.json") in the individual profile folders.
...
class IntMorphsConfigFilter:  # pylint:disable=too-many-instance-attributes
    ...
    def _get_filter_item(self, key: str, expected_type: type) -> Any:
        try:
            ...
            intmorphs_globals.new_config_found = True
        assert isinstance(filter_item, expected_type)
        return filter_item


class IntMorphsConfig:  # pylint:disable=too-many-instance-attributes, too-many-statements
    def __init__(self, is_default: bool = False) -> None:
        try:
            ...
            self.filters: list[IntMorphsConfigFilter] = self.get_config_filters(
                is_default
            )
        ...
        if (
            ...
            and not intmorphs_globals.shown_config_warning
        ):
            # show_warning_new_config_items()  # temporarily disabled
            intmorphs_globals.shown_config_warning = True

    def update(self) -> None:
        # The same IntMorphsSettings object is shared many
    ...
    def get_config_filters(
        ...
        for _filter in config_filters:
            am_filter = IntMorphsConfigFilter(_filter)
    ...
    def _get_config_item(
        ...
        intmorphs_globals.intmorphs_new_config_found = True
        """
...
def load_stored_am_configs(
    ...
    """
    This function loads the stored dict found in 'intmorphs_profile_settings.json' and
...
def save_config_to_am_file(
    ...
    profile_settings_path = Path(
        mw.pm.profileFolder(), intmorphs_globals.PROFILE_SETTINGS_FILE_NAME
    )
...
def get_read_enabled_filters() -> list[IntMorphsConfigFilter]:
    config_filters = IntMorphsConfig().get_config_filters()
...
def get_modify_enabled_filters() -> list[IntMorphsConfigFilter]:
    config_filters = IntMorphsConfig().get_config_filters()
    assert isinstance(config_filters, list)
...
def get_matching_filter(note: Note) -> IntMorphsConfigFilter | None:
    assert mw is not None
    config_filters = IntMorphsConfig().get_config_filters()
    assert isinstance(config_filters, list)
...
def get_matching_modify_filter(note: Note) -> IntMorphsConfigFilter | None:
    assert mw is not None
    modify_filters: list[IntMorphsConfigFilter] = get_modify_enabled_filters()
...
def get_matching_read_filter(note: Note) -> IntMorphsConfigFilter | None:
    assert mw is not None
    read_filters: list[IntMorphsConfigFilter] = get_read_enabled_filters()
...
def show_critical_config_error() -> None:
    ...
    critical_box.setWindowTitle("IntMorphs Error")
    ...
    body: str = (
        ...
        "<li> Restore the default configs of 'IntMorphs' (and 'intmorphs' if you have that)<br>"
        "    Tools -> add-ons -> select add-on -> config -> restore defaults"
        "<li> Delete the 'intmorphs_profile_settings.json' file in the Anki profile folder"
        "<li> Restart Anki"
        "</ol>"
    )
