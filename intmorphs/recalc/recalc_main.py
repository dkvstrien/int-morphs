...
from .anki_data_utils import IntMorphsCardData
...
def recalc() -> None:
    ...
    read_enabled_config_filters: list[IntMorphsConfigFilter] = (
        intmorphs_config.get_read_enabled_filters()
    )
    modify_enabled_config_filters: list[IntMorphsConfigFilter] = (
        intmorphs_config.get_modify_enabled_filters()
    )
...
def _check_selected_settings_for_errors(
    ...
    for config_filter in config_filters:
        ...
        if intmorphs_globals.NONE_OPTION in options_possibly_containing_none:
        ...
        if (
            config_filter.morph_priority_selection
            != intmorphs_globals.COLLECTION_FREQUENCY_OPTION
        ):
            priority_file_path = Path(
                mw.pm.profileFolder(),
                intmorphs_globals.PRIORITY_FILES_DIR_NAME,
                config_filter.morph_priority_selection,
            )
...
def _update_cards_and_notes(  # pylint:disable=too-many-locals, too-many-statements, too-many-branches
    ...
    for config_filter in modify_enabled_config_filters:
        ...
        cards_data_dict: dict[CardId, IntMorphsCardData] = (
...
def _on_failure(  # pylint:disable=too-many-branches
    ...
    title = "IntMorphs Error"

    if isinstance(error, DefaultSettingsException):
        text = (
            f'Found a note filter containing a "{intmorphs_globals.NONE_OPTION}" option. Please select something else.<br><br>'
        ...
        text = "The IntMorphs settings uses one or more note types that no longer exists. Please redo your settings."
    elif isinstance(error, AnkiFieldNotFound):
        text = "The IntMorphs settings uses one or more fields that no longer exist. Please redo your settings."
    elif isinstance(error, MorphemizerNotFoundException):
        if error.morphemizer_name == "MecabMorphemizer":
            text = (
                'Morphemizer "IntMorphs: Japanese" was not found.<br><br>'
            ...
            text = (
                'Morphemizer "IntMorphs: Chinese" was not found.<br><br>'
