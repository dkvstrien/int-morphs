...
def cache_anki_data(  # pylint:disable=too-many-locals, too-many-branches, too-many-statements
    ...
    # Rebuilding the entire intmorphs db every time is faster and much simpler than
    # updating it since we can bulk queries to the anki db.
    am_db = IntMorphsDB()
    am_db.drop_all_tables()
    am_db.create_all_tables()

    # These lists contain data that will be inserted into intmorphs.db
    ...
    progress_utils.background_update_progress(label="Saving to intmorphs.db")
...
def _get_card_memory_strength(
    am_config: IntMorphsConfig, card_data: AnkiCardData
) -> int:
...
def _get_morphs_from_files(am_config: IntMorphsConfig) -> list[dict[str, Any]]:
    assert mw is not None
...
def _get_lemma_and_inflection_columns(
    ...
    # compatible with 'known morphs' files from IntMorphs v2
...
def _get_morphs_from_minimum_format(
    am_config: IntMorphsConfig, morph_reader: Any, lemma_column: int
) -> list[dict[str, Any]]:
    morphs_from_files: list[dict[str, Any]] = []
...
def _update_learning_intervals(
    am_config: IntMorphsConfig, morph_table_data: list[dict[str, Any]]
) -> None:
