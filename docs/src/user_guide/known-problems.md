...
> IntMorphs uses the Anki API to run in the background after you answer a card, which then
...
> When using the `IntMorphs: Japanese` morphemizer, excessively long lines of text can cause the morphemizer's buffer
...
<details>
  <summary style="display:list-item">Anki crashing when opening IntMorphs settings</summary>

> The `IntMorphs: Japanese` morphemizer doesn't handle paths with diacritical marks very well, so paths like this:
