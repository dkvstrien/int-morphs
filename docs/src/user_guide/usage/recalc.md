...
Recalc is short for “recalculate”, and is basically the command that tells IntMorphs to work all its
magic. When you run Recalc, IntMorphs will go through
...
Basically, when you run Recalc, IntMorphs will go through your collection, recalculate
...
[Each morph has a priority value](../setup/prioritizing.md), which IntMorphs aggregates into the following
metrics:
\\[
{\Large
\begin{array}{ccc}
...
IntMorphs provides the following metrics, which you can influence by changing the coefficients:
