...
When you first start using IntMorphs, you will probably come across many variations of Interjections (e.g. `Aaah!`,
...
IntMorphs might seem error-prone at first, like mixing up two (seemingly) different morphs, but the more data it
...
There will also be times when IntMorphs says a card is [1T](../glossary.md#1t-sentence), but you aren’t able to
understand it. There are two reasons this may occur. The first is that, due to incorrect parsing, IntMorphs thinks you
know a word that you don’t. Unfortunately, there is no easy way to remove morphs from the IntMorphs' database. Luckily,
...
suspend/delete theccard and move on. The whole point of IntMorphs is to help you make fast progress by collecting
...
the `IntMorphs` menu at the top of the browse window.
...
There are three scenarios where IntMorphs will automatically skip a card:
...
The skipping features mentioned in the section above only take effect when using Anki on desktop where the IntMorphs
...
IntMorphs adds some additional options to the Anki context menu (right-click):
