...
> Should I use the same tags in IntMorphs that I was using with Morphman?

I recommend using the default IntMorphs tags. Mixing tags can get confusing.

> Should I export all of studied and in progress words into a CSV spreadsheet?

IntMorphs determines which morphs are known in the same way MorphMan does it: by how long the learning
...
known with MorphMan, you can usually get them back quickly by using `K` when you encounter them when using IntMorphs.


> Should I manually delete the words in the focus morph field of my cards so that IntMorphs can cleanly reparse
> everything?

IntMorphs does not reuse the MorphMan focus morph field, so it makes no difference.
