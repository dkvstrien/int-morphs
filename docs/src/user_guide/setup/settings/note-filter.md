...
IntMorphs only analyzes and sorts cards that match at least one note filter; if you don't specify any note filters,
then IntMorphs won't do anything, so this is a necessary step. This can seem overly complicated and overwhelming, but
...
might significantly improve how much you benefit from IntMorphs.
...
1. Go to Browse
2. Find a card you want IntMorphs to analyze and sort
...
You can further filter IntMorphs to only work on cards with a certain note type **and** with/without specific tag(s).
...
This is the field on the card IntMorphs reads and analyzes, which is then used to sort the card.
...
This is the tool IntMorphs uses to split text into morphs. See the [installation section](../../installation.md) for 
...
IntMorphs automatically finds `.csv` files placed
...
If, for whatever reason, you don't want IntMorphs to read one of the note filters you have set up, then you
can uncheck the `Read` option.

If you uncheck `Modify`, IntMorphs will analyze the
