...
The more cards you have with unique morph combinations, the more likely IntMorphs is to find the ideal next learning
card for you.
Large decks like those generated by [sub2srs](../glossary.md#sub2srs) are therefore well-suited for IntMorphs. The
...
Every subsequent sync should be lightning fast, even if IntMorphs makes changed to all your cards, it will usually only
...
If you have already studied cards and are comfortable with their content, you can configure IntMorphs
...
deck, it's probably better to have IntMorphs read the word field, and not the example sentence field.
...
the **“read”** option and IntMorphs will then stop treating those morphs as known.
