...
behind IntMorphs--learn a language in the order that will be the most useful.

IntMorphs is a general purpose language learning tool, therefore, it has to be told which morphs occur most often. You
can do this in two ways, either have IntMorphs calculate the morph frequencies found in your
...
> - Any lines after 1 million will be ignored by IntMorphs
...
> **Note**: This is a lemma only priority file that was **not** generated using IntMorphs, so it might not work very well (or at all).
...
>    - Morphemizer: `IntMorphs: Japanese`
